...
export default function DeleteAccessLogModal({ isOpen, log, onClose, onUnauthorized, onDeleted }) {
  const resolvedLogId = log?.id
  ...
  const handleDelete = async () => {
    if (!resolvedLogId) {
      setErrorMessage('Missing log id.')
      return
    }
    ...
    try {
      await api.delete(`/access-logs/${resolvedLogId}/`)
      onDeleted && onDeleted(resolvedLogId)
      onClose && onClose()
      return
    } catch (err) {
      ...
      if (status === 403) {
        setErrorMessage(err.response?.data?.detail || 'Forbidden. You do not have permission to delete this log.')
      } else if (status === 404) {
        setErrorMessage(err.response?.data?.detail || 'Log not found.')
      } else {
        setErrorMessage(err.response?.data?.detail || 'Failed to delete log.')
      }
    } finally {
      setSaving(false)
    }
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-[520px] rounded-2xl bg-white p-8 shadow-xl">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-[#0F172A]">Delete Access Log</h2>
            <p className="mt-1 text-sm text-[#6B7280]">Are you sure you want to permanently delete this access log?</p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-900">{log?.role || '—'}</p>
          </div>
          <div className="sm:col-span-2">
            ...
            <p className="mt-1 text-sm text-slate-900">
              {log?.login_time
                ? new Intl.DateTimeFormat('en-US', {
                    dateStyle: 'medium',
                    timeStyle: 'short',
                  }).format(new Date(log.login_time))
                : '—'}
            </p>
          </div>
        </div>
