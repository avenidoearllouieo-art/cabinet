...
import { Send, User, Clock3, Paperclip, Edit3, Trash2 } from 'lucide-react'
    ...
    <section className="relative flex h-[85vh] max-h-[85vh] min-h-[520px] flex-col overflow-hidden rounded-[24px] border border-slate-200/80 bg-white/95 shadow-[0_20px_60px_-24px_rgba(15,23,42,0.2)]">
      ...
      <div className="flex-shrink-0 border-t border-slate-200/80 bg-white px-6 py-4">
        ...
        <div className="flex flex-col gap-3 sm:flex-row sm:items-end">
          <label className="sr-only" htmlFor="discussion-message">Write a message</label>
          <textarea
            ...
            className="min-h-[84px] flex-1 resize-none rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-900 outline-none transition focus:border-blue-400 focus:ring-2 focus:ring-blue-100"
          />
          <div className="flex flex-col gap-3 sm:w-auto sm:flex-row">
            <label className="inline-flex cursor-pointer items-center gap-2 rounded-full border border-slate-200 bg-white px-4 py-3 text-sm font-semibold text-slate-700 transition hover:bg-slate-50">
            ...
            <button
              ...
              className="inline-flex items-center justify-center gap-2 rounded-full bg-blue-600 px-5 py-3 text-sm font-semibold text-white transition hover:bg-blue-700 disabled:cursor-not-allowed disabled:opacity-60"
            >
        ...
        {attachment && (
          <div className="mt-3 flex items-center justify-between rounded-[18px] border border-slate-200 bg-slate-50 px-4 py-3 text-sm text-slate-700">
            ...
            <button type="button" onClick={() => setAttachment(null)} className="font-semibold text-blue-600 hover:text-blue-800">Remove</button>
          </div>
        )}
      </div>
    </section>
