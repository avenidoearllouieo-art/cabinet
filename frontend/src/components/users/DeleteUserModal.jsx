...
export default function DeleteUserModal({ isOpen, user, onClose, onUnauthorized, onDeleted }) {
  ...
  const handleDelete = async () => {
    ...
    try {
      ...
      if (status === 403) {
        setErrorMessage(err.response?.data?.detail || 'Forbidden. You do not have permission to delete this user.')
      } else if (status === 404) {
        setErrorMessage(err.response?.data?.detail || 'User not found.')
      } else {
    ...
    <div className="fixed inset-0 z-[9999] flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
      <div className="w-full max-w-[520px] rounded-2xl bg-white p-8 shadow-xl">
        <div className="mb-4 flex items-start justify-between gap-4">
          <div>
            <h2 className="text-2xl font-semibold text-[#0F172A]">Delete User</h2>
            <p className="mt-1 text-sm text-[#6B7280]">Are you sure you want to permanently delete this user?</p>
          </div>
        ...
        <div className="grid gap-4 sm:grid-cols-2">
          <div>
            ...
            <p className="mt-1 text-sm text-slate-900">{user?.student_id || '—'}</p>
          </div>
          <div>
          ...
          <div>
            ...
            <p className="mt-1 text-sm text-slate-900">{fullName}</p>
          </div>
          <div>
